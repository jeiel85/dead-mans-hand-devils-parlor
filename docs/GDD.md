...
- 남은 시간은 벽시계가 아니라 프레임 델타로 센다. 한 프레임이 먹을 수 있는 시간은 0.25초로 제한하고, 창이 포커스를 잃으면 멈춘다. 브라우저 탭을 숨겼다 돌아왔을 때 도전 창이 통째로 소모돼 대신 응답되는 것을 막기 위해서다.
### 12.3 격발 연출 ✅(프로토타입 수준)
...
### 12.5 로컬라이징 ✅
- 한국어 원문, 영어 병행. 문자열 키 기반(`src/i18n.js`, `godot/scripts/core/i18n.gd`). 조사는 문자열에 두 형태를 모두 적어 두고(`{who}이(가)`) 치환 뒤 앞 음절의 받침으로 고른다(이/가·을/를·은/는·와/과·으로/로). 받침을 판별할 수 없는 앞글자(라틴 문자·숫자)는 병기를 그대로 두어 틀린 조사를 만들지 않는다.
